...
struct EnemyBox
{
    ...
    */
    float intersectDown(EnemyBox& other)
    {
    ...
    float intersectUp(EnemyBox& other)
    {
        if (fabs(m_pos_x - other.m_pos_x) * 2.0f >= (m_width + other.m_width) || m_pos_y < other.m_pos_y)
            return 0.0f;
    ...
    *   anymore with the target Box (other), or 0.0f if no collision is found.
    */
    float intersectSideways(EnemyBox& other)
    {
    ...
    */
    EnemyBox(float x, float y, float w, float h)
