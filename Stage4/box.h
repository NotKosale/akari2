...
struct Box
{
    ...
    */
    ...
    float intersectRight(Box& other)
    {
        if (fabs(m_pos_y - other.m_pos_y) * 2.0f >= (m_height + other.m_height) || m_pos_x <= other.m_pos_x)
            return 0.0f;
    ...
    bool isIntersecting(Box& other)
    {
        bool horizontalIntersection = fabs(m_pos_x - other.m_pos_x) * 2.0f < (m_width + other.m_width);
        bool verticalIntersection = fabs(m_pos_y - other.m_pos_y) * 2.0f < (m_height + other.m_height);
